...
static void aes_encrypt(ut_runner *r)
{
	x_aes_ctx ctx;
	uint8_t in[16];
	uint8_t plain[]  = { 0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a };
	{
		...
		memcpy(in, plain, sizeof in);
		x_aes_init(&ctx, key, 128);
		x_aes_encrypt(&ctx, in);
		ut_assert_mem_equal(r, in, 16, out, 16);
	}
	...
	
	{
		...
		memcpy(in, plain, sizeof in);
		x_aes_init(&ctx, key, 256);
		x_aes_encrypt(&ctx, in);
		ut_assert_mem_equal(r, in, 16, out, 16);
	}
}

static void aes_decrypt(ut_runner *r)
{
	x_aes_ctx ctx;
...
static void cbc_encrypt(ut_runner *r)
{
	uint8_t in[64];
    x_aes_ctx ctx;
...

	{
		...
		x_aes_init(&ctx, key, 256);
		x_aes_set_iv(&ctx, iv);
		x_aes_cbc_encrypt(&ctx, in, 64);
		ut_assert_mem_equal(r, out, 64, in, 64);
	}
...
static void cbc_decrypt(ut_runner *r)
{
    x_aes_ctx ctx;
	uint8_t iv[]  = { 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f };
    uint8_t out[] = { 0x6b, 0xc1, 0xbe, 0xe2, 0x2e, 0x40, 0x9f, 0x96, 0xe9, 0x3d, 0x7e, 0x11, 0x73, 0x93, 0x17, 0x2a,
...
static void ctr_xcrypt(ut_runner *r)
{
	x_aes_ctx ctx;
...
void aes_test_init(ut_suite *s)
{
	ut_suite_init(s, "aes.h");
	ut_suite_add(s, aes_encrypt);
	ut_suite_add(s, aes_decrypt);
	ut_suite_add(s, cbc_encrypt);
	ut_suite_add(s, cbc_decrypt);
	ut_suite_add(s, ctr_xcrypt);
}
